    ...
    private void load(HttpExchange httpExchange) throws IOException {
        try {
            ...
            if ("GET".equals(httpExchange.getRequestMethod())) {
                ...
                if (key.isEmpty()) {
                    System.out.println("Ключ пуст, невозможно получить значение. Ключ должен быть указан: /load/{ключ}");
                ...
                System.out.println("/load ожидает GET-запроса, но получил: " + httpExchange.getRequestMethod());
    ...
    private void save(HttpExchange httpExchange) throws IOException {
        try {
            ...
            if ("POST".equals(httpExchange.getRequestMethod())) {
                ...
                if (key.isEmpty()) {
                    System.out.println("Ключ пуст, невозможно получить значение. Ключ должен быть указан: /save/{ключ}");
                    httpExchange.sendResponseHeaders(400, 0);
                    return;
                }
                String value = readText(httpExchange);
                if (value.isEmpty()) {
                    System.out.println("Значение пустое, сохранить не удается. Значение должно быть указано в основном запросе");
                    httpExchange.sendResponseHeaders(400, 0);
                    return;
                }
                data.put(key, value);
                ...
                System.out.println("/save ожидает POST-запроса, но получил: " + httpExchange.getRequestMethod());
                httpExchange.sendResponseHeaders(405, 0);
            }
        } finally {
            httpExchange.close();
    ...
    private void register(HttpExchange httpExchange) throws IOException {
        try {
            ...
            if ("GET".equals(httpExchange.getRequestMethod())) {
                ...
                System.out.println("/register ожидает GET-запроса, но получил " + httpExchange.getRequestMethod());
                httpExchange.sendResponseHeaders(405, 0);
            }
        } finally {
            httpExchange.close();
